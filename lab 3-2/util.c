...
void write_memory(unsigned int data_size, unsigned int MAR, int MDR) {
	switch(data_size & 0x3) {
		case 0:
		case 1:
            MEMORY[MAR] = MASK7_0(MDR);
            MEMORY[MAR + 1] = MASK15_8(MDR);
            MEMORY[MAR + 2] = MASK23_16(MDR);
            MEMORY[MAR + 3] = MASK31_24(MDR);
			break;
		case 2:
            MEMORY[MAR] = MASK7_0(MDR);
            MEMORY[MAR + 1] = MASK15_8(MDR);
...
int read_memory(unsigned int data_size, unsigned int MAR) {
	...
	switch(data_size & 0x3) {
		case 0:
		case 1:
            MemOut = (MEMORY[MAR + 3] << 24) + \
                (MEMORY[MAR + 2] << 16) + \
                (MEMORY[MAR + 1] << 8) + \
                MEMORY[MAR];
			break;
		case 2:
           MemOut = blockSEXT((MEMORY[MAR + 1] << 8) + \
                MEMORY[MAR], 16);
			break;
		case 3:
            MemOut = blockSEXT(MEMORY[MAR], 8);
			break;
	}
	return MemOut;
}
