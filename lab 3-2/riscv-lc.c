...
void eval_bus_drivers() {
    ...
    int* microInstr = CURRENT_LATCHES.MICROINSTRUCTION;
    int ir = CURRENT_LATCHES.IR;
    int adder = 0, alu = 0, shf = 0;

    int sextImm = blockSEXT(mask_val(ir, 31, 20), 12);
    int block_s_type = blockSEXT(blockStypeImm(mask_val(ir, 11, 7), mask_val(ir, 31, 25)), 12);
    int block_j_type = blockSEXT(blockJtypeImm(mask_val(ir, 31, 31), mask_val(ir, 30, 21), mask_val(ir, 20, 20), mask_val(ir, 19, 12)), 20);
    adder = blockADDR2MUX(get_ADDR2MUX(microInstr), 0, sextImm, block_s_type, block_j_type);


    int _rs1 = blockRS1En(get_RS1En(microInstr), 0, CURRENT_LATCHES.REGS[mask_val(ir, 19, 15)]);
    
    int block_b_type = blockSEXT(blockBtypeImm(mask_val(ir, 7, 7), mask_val(ir, 11, 8), mask_val(ir, 30, 25), mask_val(ir, 31, 31)), 12);
    adder += blockADDR1MUX(get_ADDR1MUX(microInstr), 0, CURRENT_LATCHES.PC, _rs1, block_b_type);
    ...
    int funct3 = mask_val(ir, 14, 12);
    int _imm12 = mask_val(ir, 31, 20);
    int funct7 = mask_val(ir, 31, 25);
